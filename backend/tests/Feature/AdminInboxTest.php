...
class AdminInboxTest extends TestCase
{
    ...
    public function test_conversation_messages_display_chronologically(): void
    {
        ...
        $messageStream = substr($content, strpos($content, 'message-day-label'));
    ...
    public function test_valid_outbound_message_is_stored(): void
    {
        ...
        $whatsapp->shouldReceive('sendText')
            ->once()
            ->with($conversation->customer->phone, 'Your mattress consultation is confirmed.')
            ->andReturn([]);
        $whatsapp->shouldReceive('isConfigured')->once()->andReturn(false);
    ...
    public function test_empty_outbound_messages_are_rejected(): void
    {
        ...
        $this->actingAs($admin)
            ->from(route('admin.inbox', ['conversation' => $conversation->id]))
            ->post(route('admin.inbox.messages.store', $conversation), ['body' => ''])
            ->assertRedirect(route('admin.inbox', ['conversation' => $conversation->id]))
